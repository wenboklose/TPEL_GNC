...
figHandle=figure(211);
set(figHandle, 'Position', [50, 10, 1000, 800]);
plot(Leigenvalues(2,:)*exp(-i*1.7),linestyle1,'LineWidth',linewidth)
hold on
legend({'{\it\lambda}_{1}','{\it\lambda}_{1}_{mirror}','{\it\lambda}_{2}','{\it\lambda}_{2}_{mirror}'},'Fontsize',fontsize,'FontWeight','bold')
plot(-1,0,'r+','LineWidth',linewidth)
% hold off
...
point1=[-3*cos(pi/3) -0.5*cos(pi/3)]; point2=[3*sin(pi/3) 0.5*sin(pi/3)]; point3=[-0.5*cos(pi/3) -3*cos(pi/3)]; point4=[-0.5*sin(pi/3) -3*sin(pi/3)];
line(point1,point2,'LineWidth',linewidth);
line(point3,point4,'LineWidth',linewidth);

r=0.5;
theta=linspace(2*pi/3,4*pi/3,1000);
x=r*cos(theta);
y=r*sin(theta);
...
% plot the full GNC Characteristic Loci
figHandle=figure(22);
set(figHandle, 'Position', [1000, 10, 1000, 800]);
plot(Leigenvalues(1,:),linestyle1,'LineWidth',linewidth)
hold on
