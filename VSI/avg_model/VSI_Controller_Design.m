...
% tfidd_nl_delay =  tfidd_nl*tfdelay;
...
figure(4)
step(tf_i_cl,0.05)										% step response
figure(5)
bode(T_i_nl,P)
grid on
Bode_Darklines(3);
title('Current Control Loop Gain')
figure(6)
bode(tf_i_cl,tfididref,P)
grid on
Bode_Darklines(3);
title('Current Control Loop Gain')
tfvdid_nl=tfvdd_nl_del/tfidd_nl_del;
tfidrefvd_nl=tf_i_cl*tfvdid_nl;
tfidrefvd_nl_del=tfidrefvd_nl*tfdelay;
tf_idvd=tf([0 Rac],[C*Rac 1]);

% figure(7)
% bode(tfidrefvd_nl,tfvdidref,tf_idvd)

figure(7)
bode(tfidrefvd_nl_del);


Kpv=2*pi*300*C
Kiv=2*pi*300/Rac
tf_v_c=tf([Kpv  Kiv],[1  0]);
T_v_nl=tfidrefvd_nl_del*tf_v_c;
tf_v_cl=feedback(T_v_nl,1,-1);						% transfer function bw i_ref and i
figure(8)
step(tf_v_cl,0.05)										% step response
figure(9)
bode(T_v_nl,P)
grid on
Bode_Darklines(3);
title('Voltage Control Loop Gain')
figure(10)
bode(tf_v_cl,P)
grid on
Bode_Darklines(3);
